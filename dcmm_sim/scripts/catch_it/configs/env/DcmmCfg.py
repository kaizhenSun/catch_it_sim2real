...
# Use Leap Hand
XML_ARM_PATH = "urdf/piper_no_gripper_description.xml"
XML_ARM_RAW_PATH = "urdf/piper_no_gripper_description_mini.xml"
## Weight Saved Path
...
# object_static = np.array([1.5, 1.75])
## Observation Noise
k_obs_base = 0.01
k_obs_arm = 0.001
k_obs_object = 0.01
...
# 比较好的一组PID
# Kp_arm = np.array([50.0, 100.0, 100.0, 2.5, 10.0, 5.0])
# Ki_arm = np.array([1e-2, 1, 1, 1, 5, 1e-3])
# Kd_arm = np.array([5, 5, 5, 0.1, 1, 0.1])
# llim_arm = np.array([-100.0, -100.0, -100.0, -50.0, -50.0, -20.0])
# ulim_arm = np.array([100.0, 100.0, 100.0, 50.0, 50.0, 20.0])
# 比较不错的一组PID
# Kp_arm = np.array([100.0, 100.0, 100.0, 10, 10.0, 5.0])
# Ki_arm = np.array([1e-2, 1e-2, 1e-2, 5, 5, 5])
# Kd_arm = np.array([5, 10, 5, 0.1, 0.5, 0.1])
# llim_arm = np.array([-100.0, -100.0, -100.0, -50.0, -50.0, -20.0])
# ulim_arm = np.array([100.0, 100.0, 100.0, 50.0, 50.0, 20.0])

Kp_arm = np.array([100.0, 200.0, 100.0, 50, 50.0, 20.0])
Ki_arm = np.array([1e-2, 30, 30, 5, 10, 1])
Kd_arm = np.array([3.5, 10, 5, 1, 1, 0.5])
llim_arm = np.array([-100.0, -100.0, -100.0, -50.0, -50.0, -20.0])
ulim_arm = np.array([100.0, 100.0, 100.0, 50.0, 50.0, 20.0])
...
hand_joint_names = ["1", "0", "2", "3", "5", "4", "6", "7", "9", "8", "10", "11", "12", "13", "14", "15"]