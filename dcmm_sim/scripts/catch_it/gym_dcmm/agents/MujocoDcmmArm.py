...
class MJ_DCMM(object):
    ...
    def __init__(self, 
        ...
        # If model is provided, use it. Otherwise, load the model from XML file.
        if model is None:
            model_path = os.path.join(DcmmCfg.ASSET_PATH, DcmmCfg.XML_ARM_PATH)
        ...
        if model_arm is None:
            model_arm_path = os.path.join(DcmmCfg.ASSET_PATH, DcmmCfg.XML_ARM_RAW_PATH)
        ...
        self.arm_indices = self._get_joint_indices(self.model, DcmmCfg.arm_joint_names)
        self.hand_indices = self._get_joint_indices(self.model, DcmmCfg.hand_joint_names)
        self.object_indices = self._get_joint_indices(self.model, object_name)
        ...
        # self._print_joint_qpos_info(self.model)

        self.data.qpos[0:6] = DcmmCfg.arm_joints[:]
        self.data.qpos[6:22] = DcmmCfg.hand_joints[:]
        ...
        # Get the geom id of the floor and the object
        try:
    ...
    def set_throw_pos_vel(self, 
        ...
        jid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, self.object_name)
        if jid < 0:
            raise ValueError(f"Joint '{self.object_name}' not found")
        if self.model.jnt_type[jid] != mujoco.mjtJoint.mjJNT_FREE:
            raise ValueError(f"Joint '{self.object_name}' is not a free joint")
    ...
    def create_camera_data(self, width, height, camera):
        ...
        self.cam_pos = self.model.cam_pos0[cam_id] - self.data.body("arm_base").xpos
        self.cam_init = True
    def _get_joint_indices(self, model, joint_names):
        """返回 joint_names 在 qpos 里的 index 列表"""
        indices = []
    ...
    def _print_joint_qpos_info(self, model):
        """打印所有关节的名称及其对应的 qpos 地址和数量"""
        for i in range(model.njnt):
            ...
            # 打印关节名称，qpos 地址和数量
            print(f"{jname:10s} -> qpos[{qpos_adr}]")
